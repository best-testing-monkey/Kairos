...
- **Persistent prediction cache** (`strategy/kairos_predcache.py`): as of 2026-08-11, primarily a `SqliteDict`-backed table (`caches.db`) at `data/predcache/` (env `KAIROS_PRED_CACHE_DIR`) — `get()`/`has()`/`put()` all check/write it first. A legacy `.npz`-per-key disk store and a byte-bounded in-memory LRU still exist underneath (see the module docstring for the current three-layer order), but `put()` no longer writes through to the LRU at all — it's populated only as a `get()`-side-effect on a disk-only hit. Keys include symbol/interval/bar/lookback/`pred_len`/`pred_samples`/model/content-hash/`checkpoint_fingerprint` — the fingerprint (size+mtime of `model.safetensors`) busts the cache when a finetuned checkpoint is retrained in place. Disk is bounded (default 2 GiB, oldest-mtime eviction; `KAIROS_PRED_CACHE_MAX_BYTES` to tune) — **but eviction only deletes `.npz` files, not `caches.db` itself**, so an oversized sqlite file can leave eviction unable to ever bring the total back under budget (known issue, not yet fixed). `kairos_papertrade.py --no-pred-cache` disables prewarm/reuse for a run. `kairos_pipeline.py --stage auto` uses the same module with a *separate ephemeral* tempdir — don't conflate the two.
- **Use `.has()`, not `.get()`, for existence checks.** `PredictionCache.get()` promotes a disk hit into the in-memory LRU as a side effect; calling it as a boolean probe (as `is_batch_cached()` once did) silently grows RAM until the machine freezes. `.has()` never touches `_mem` — except on a disk-only hit (sqlite and mem both missed), where it now calls `get()` internally to backfill sqlite, so it's no longer *unconditionally* side-effect-free.
- **Model-major prewarm, single inline-checked pass** (changed 2026-08-11): `kairos_papertrade.main()` runs `prewarm_prediction_cache()` before the date-major report loop so each model (base + each finetuned group) loads exactly once per run. `kairos_strategies.predict_all_batch` defers `_materialize_model` until *after* the shared-cache lookup and skips it on a full hit. There is no separate check-then-load pass anymore — the old two-phase design (check pass stops at first miss, load pass only runs if needed) was replaced with one pass that calls `is_batch_cached()` inline per entry and skips predicting on a hit. The periodic `gc.collect()` that used to live in the (now-commented-out) check-pass loops is currently dormant as a result — see CLAUDE.md.
- **`_prediction_cache` removed entirely (2026-08-11)**, not just capped. It used to be a per-process dict in `kairos_strategies.py` (distinct from `kairos_predcache`) capped at 5000 entries (`_PREDICTION_CACHE_MAX_ENTRIES`, cleared on overflow) after a 2026-07-30 leak fix. Now every symbol lookup/write goes straight through the shared `kairos_predcache`; a much smaller `_shared_keys: dict[str, str]` (symbol → cache key) replaced it, purely to avoid recomputing the cache key later in the same call. `_dist_cache` (a different, still-present per-process dict — `KairosDistribution` objects, not raw predictions) is separately capped the same way via `_DIST_CACHE_MAX_ENTRIES`/`_dist_cache_put()`, and was in fact the dominant leak among three overlapping caches — see CLAUDE.md's dated section for the full investigation.
- **In-process RSS safety net**: `strategy/memory_monitor_heap.py`, imported unconditionally near the top of `kairos_papertrade.py`, runs a daemon thread that polls RSS every 0.5s and past 6000MB dumps the top `tracemalloc` allocation sites under this repo's path and hard-exits (`os._exit(1)`) — added after repeated live-debugging freezes; see CLAUDE.md.
- **This whole area is WIP** (commit message: "cashing fixes WIP — still need to remove old predcache") and currently breaks 48 tests across `test_predcache.py`/`test_kairos_strategies_model_switch.py`/`test_predict_all_batch_cache.py` (stale references to the removed `_prediction_cache`, plus a real disk-eviction regression), and has a live `NameError` bug (`_no_data_fallback_warned` got accidentally nested inside `_dist_cache_put()`, so it's no longer a module-level binding). See CLAUDE.md's dated section for the full list before building on this code.
...
- **Report de-dup is persistent**: `generate_and_dedupe_reports()` keeps its `seen` map in a `SqliteDict` (`report_seen.db`, table `seen_v2_<hash>` for new runs). The v2 hash covers base_now/interval/work-item groups **plus accepted-finetuned model paths**, so a newly accepted finetuned model for an existing group busts already-seen dates. Existing `seen_<legacy_hash>` tables are read as a fallback so in-flight runs do not suddenly regenerate everything. Two caveats: the filename is **CWD-relative** — run from the repo root or you get a fresh, empty DB — and the legacy fallback means a window that already has a populated legacy table will keep using it until that table is deleted or a v2 table is seeded for the same window.
