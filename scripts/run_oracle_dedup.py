#!/usr/bin/env python3
"""Run --stage oracle over a deduped representative subset of suggested_groups.

Runs `select_deduped_groups()` through `kairos_pipeline.run_stage_oracle()`
across a process pool of workers instead of one group at a time. Supersedes
the original sequential scratchpad version of this script (same name, used
2026-08-26): that version processed one group at a time via a blocking
`subprocess.run()` per group, which measured (via `ps`) at only ~4 cores'
worth of BLAS work on average on a 16-logical-core box -- most cores sat idle
(the "one core pegged, others 5-40%" pattern was Linux's scheduler
load-balancing that one hot process across cores, not real parallelism).
Root cause turned out to be mostly single-threaded Python (the day-by-day,
strategy-by-strategy loop is GIL-bound, not BLAS-bound) -- confirmed via a
controlled 4-vs-4-group benchmark: 4 parallel workers gave a 3.78x speedup
over the sequential approach, close to linear. Each worker subprocess is
pinned to 1 BLAS thread so N workers approximate N busy cores instead of
oversubscribing.
...
Usage:
    uv run scripts/run_oracle_dedup.py <correlation_run_id> [--workers N] [--limit N]
"""
import argparse
import os
import sqlite3
...
def main():
    ...
    print(f"Total oracle invocations planned: {len(groups)} across {args.workers} workers")

    done = skipped = failed = 0
    t0 = time.time()
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        futures = {pool.submit(_run_one, gid, assets): (gid, assets) for gid, assets in groups}
    ...
    print(f"\nParallel oracle sweep done: {done} run, {skipped} skipped, {failed} failed, "
          f"total={len(groups)}.")
