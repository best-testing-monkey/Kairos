...
/graphify <path> --no-semantic                        # AST-only: zero-token build, no LLM subagents (code structure only)
...
**`--no-semantic` flag:** If the user passed `--no-semantic`, SKIP Part B entirely regardless of corpus contents — write an empty `graphify-out/.graphify_semantic.json` (`{"nodes":[],"edges":[],"hyperedges":[],"input_tokens":0,"output_tokens":0}`) and go straight to Part C. Docs/papers/images are simply left out of the graph. In Step 5, label communities from node names alone (module/class names are usually self-explanatory) — this costs only a handful of tokens in the main session, no subagents. The result is a structural code graph (imports, calls, classes) built for ~zero tokens; note in the final summary that doc/semantic edges were skipped and can be added later with `--update` (cached AST results are reused, only semantic work runs).

**Fast path:** If detection found zero docs, papers, and images (code-only corpus), skip Part B entirely and go straight to Part C. AST handles code - there is nothing for semantic subagents to do.

**Token-budget guard:** Before dispatching, sanity-check the uncached non-code file list. If it contains files under `venv/`, `.venv/`, `node_modules/`, `data/`, build output, or generated artifacts, exclude them (they poison the graph AND burn tokens). If more than ~8 agent chunks would be needed and no `GEMINI_API_KEY`/`GOOGLE_API_KEY` is set, STOP and tell the user the estimated chunk count, recommending either (a) setting `GEMINI_API_KEY` so extraction is near-free, or (b) narrowing to a subfolder, or (c) proceeding in Haiku batches. Wait for their choice — do not burn the session's budget by default.
...
**IMPORTANT - model:** Always pass `model: "haiku"` on every extraction Agent call. Extraction follows a fixed JSON schema and needs no judgment — never let these agents inherit the (expensive) parent model. This is mandatory per the user's `cheap-models-first` policy.
